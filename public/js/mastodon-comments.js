...
`;
  var MastodonComments = class extends HTMLElement {
    constructor() {
      super();
    ...
    toot_stats(toot) {
      ...
      <div class="replies ${this.toot_active(toot, "replies")}">
        <a href="${toot.url}" rel="ugc nofollow"><i class="fa fa-reply fa-fw"></i>${this.toot_count(
        toot,
        "replies"
      )}</a>
      </div>
      <div class="reblogs ${this.toot_active(toot, "reblogs")}">
        <a href="${toot.url}/reblogs" rel="nofollow"><i class="fa fa-retweet fa-fw"></i>${this.toot_count(
        toot,
        "reblogs"
      )}</a>
      </div>
      <div class="favourites ${this.toot_active(toot, "favourites")}">
        <a href="${toot.url}/favourites" rel="nofollow"><i class="fa fa-star fa-fw"></i>${this.toot_count(
        toot,
        "favourites"
      )}</a>
      </div>
    ...
    render_toots(toots, in_reply_to) {
      var tootsToRender = toots.filter((toot) => toot.in_reply_to_id === in_reply_to).sort((a, b) => a.created_at.localeCompare(b.created_at));
    ...
    `;
      ...
      li.innerHTML = typeof DOMPurify !== "undefined" ? DOMPurify.sanitize(mastodonComment.trim()) : mastodonComment.trim();
      if (toot.in_reply_to_id === this.tootId) {
        ...
        if (parentToot) {
          ...
          document.getElementById(toot.in_reply_to_id).appendChild(ul).appendChild(li);
        }
      }
    ...
    loadComments() {
      ...
      let _this = this;
      fetch("https://" + this.host + "/api/v1/statuses/" + this.tootId).then((response) => response.json()).then((toot) => {
